...
- **Attachments, the local way**: attach or drag any file (audio, video, text, whatever) onto the input. It lands in the session's `allegati/` folder, the message cites its path, and the agent goes to get it with its own tools. Small text files inline directly in plain chats.
- **It equips itself**: if a task needs something it does not have (open a PDF, OCR an image, convert media), the agent searches trusted registries (npm, PyPI, GitHub, Hugging Face), reads the docs, explains its pick, and proposes the install. **Every install still goes through your approval click**: it can want things, it cannot take them.
- **Plain chat mode**: sessions without a folder, no file tools, just talk to the model. One button.
- **Self-improvement**: the "Improve Galatea" button opens a session on Galatea's own source code, with her architecture and style rules in the prompt. Ask the model to change her UI and it edits the app you are using. The statue sculpts herself. It works locally only: pushing to the original repository is blocked at code level, contributions go through your own fork.
- **Scheduled tasks**: recurring prompts (every N hours, daily, weekly) that run in a session while the app is open. Shell commands are always denied in scheduled runs; connectors only if you allowed them when creating the task. Requires keys saved on this PC (Settings).
- **Connectors (MCP)**: a generic MCP stdio client. Declare any server in the Connectors panel and its tools appear to the model, every call gated by your approval. Two zero-dependency servers ship in `mcp-servers/`: **Gmail** (send email via app password, SMTP) and **WhatsApp** (Meta Cloud API). Community MCP servers work too.
- **One-click update**: the Update button pulls the latest version from GitHub (git pull if cloned, tarball otherwise) and tells you to restart.
- **Self-verification and checkpoints**: every file the agent modifies is backed up at the start of the turn; the `annulla_modifiche` tool rolls the whole turn back. Read-only verification commands (`node --check`, `git status/diff/log`) run without approval so the agent can actually check its own work, everything else still needs your click.
- **Sessions that never forget**: past the context window, the older part of long sessions is automatically compressed into a summary instead of falling off a cliff.
- **Hard monthly budget**: set a dollar cap in Settings and the server refuses to start any turn beyond it, scheduled runs included.
- **Event triggers**: tasks can fire when files change in the session folder (60s debounce) or via a local webhook (`POST /api/trigger/<id>`) from n8n, scripts, anything.
- **Start with Windows**: `avvio-automatico.ps1` installs a hidden logon task so scheduled work runs without opening anything.
- **Anonymizer powered by [rizzo-pii](https://github.com/Rizzo-AI-Academy/rizzo-pii)**: PII detection is done entirely by Simone Rizzo's local ML model (22 categories, runs on `127.0.0.1:5005`). Galatea auto-detects it, renumbers its placeholders into a stable per-project map so they stay reversible across messages, translates answers back locally, and writes real values to disk. If the anonymizer is ON and rizzo-pii is not running, **nothing is sent**: no fake fallback protection. Install it from [his official releases](https://github.com/Rizzo-AI-Academy/rizzo-pii/releases/latest) and go star the project: the detection brain is his, the plumbing is ours.
...
- **Allegati alla maniera locale**: alleghi o trascini qualsiasi file (audio, video, testo, quello che vuoi) sulla casella. Finisce nella cartella `allegati/` della sessione, il messaggio ne cita il percorso, e l'agente se lo va a prendere con i suoi strumenti. I file di testo piccoli entrano inline anche nelle chat.
- **Si attrezza da sola**: se un compito richiede qualcosa che non ha (aprire un PDF, fare OCR su un'immagine, convertire media), l'agente cerca sui registri affidabili (npm, PyPI, GitHub, Hugging Face), legge la documentazione, spiega la scelta e propone l'installazione. **Ogni install passa comunque dal tuo click di approvazione**: può desiderare le cose, non può prendersele.
- **Modalità chat**: sessioni senza cartella, niente strumenti sui file, solo conversazione col modello. Un bottone.
- **Automiglioramento**: il bottone "Improve Galatea" apre una sessione sul codice sorgente di Galatea stessa, con architettura e regole di stile nel prompt. Chiedi al modello di cambiarle l'interfaccia e lui modifica l'app che stai usando. La statua si scolpisce da sola. Lavora solo in locale: il push verso il repository originale è bloccato a livello di codice, i contributi passano dal tuo fork.
- **Attività ricorrenti**: prompt schedulati (ogni N ore, giornalieri, settimanali) che girano in una sessione finché l'app è aperta. I comandi shell sono sempre negati nelle esecuzioni schedulate; i connettori solo se li hai autorizzati creando l'attività. Servono le chiavi salvate sul PC (Impostazioni).
- **Connettori (MCP)**: un client MCP stdio generico. Dichiari qualsiasi server nel pannello Connectors e i suoi strumenti compaiono al modello, ogni chiamata passa dalla tua approvazione. In `mcp-servers/` ne arrivano due a zero dipendenze: **Gmail** (invio email via app password, SMTP) e **WhatsApp** (Cloud API di Meta). Funzionano anche i server MCP della community.
- **Aggiornamento con un click**: il bottone Update scarica l'ultima versione da GitHub (git pull se clonata, tarball altrimenti) e ti dice di riavviare.
- **Auto-verifica e checkpoint**: ogni file che l'agente modifica viene salvato a inizio turno; lo strumento `annulla_modifiche` riporta indietro tutto il turno. I comandi di verifica in sola lettura (`node --check`, `git status/diff/log`) girano senza approvazione, così l'agente può controllarsi il lavoro davvero; tutto il resto continua a passare dal tuo click.
- **Sessioni che non dimenticano**: oltre la finestra di contesto, il pezzo vecchio delle sessioni lunghe viene compresso in un riassunto invece di cadere nel vuoto.
- **Tetto mensile duro**: metti un tetto in dollari nelle Impostazioni e il server rifiuta di iniziare qualsiasi turno oltre quello, esecuzioni schedulate comprese.
- **Trigger a eventi**: le attività possono partire quando cambiano i file della cartella (debounce 60s) o via webhook locale (`POST /api/trigger/<id>`) da n8n, script, qualsiasi cosa.
- **Parte con Windows**: `avvio-automatico.ps1` installa un'attività nascosta all'accesso, così il lavoro schedulato gira senza aprire niente.
- **Anonimizzatore con dentro [rizzo-pii](https://github.com/Rizzo-AI-Academy/rizzo-pii)**: la rilevazione dei dati personali la fa interamente il modello ML locale di Simone Rizzo (22 categorie, gira su `127.0.0.1:5005`). Galatea lo trova da sola, rinumera i suoi segnaposto in una mappa stabile per progetto così restano reversibili tra i messaggi, ritraduce le risposte in locale e scrive su disco i valori veri. Se l'anonimizzatore è acceso e rizzo-pii non gira, **non parte niente**: nessuna finta protezione di riserva. Installalo dalle [sue release ufficiali](https://github.com/Rizzo-AI-Academy/rizzo-pii/releases/latest) e mettigli una stella: il cervello della rilevazione è suo, l'idraulica è nostra.
